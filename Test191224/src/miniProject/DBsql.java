...
public class DBsql {
	...
	public void tourReservationDB() {
		...
		try {
			...
			sql = "SELECT TOURNO, TO_CHAR(SDATE,'YYYY/MM/DD') AS SDATE, TO_CHAR(FDATE,'YYYY/MM/DD') AS FDATE, TOURNAME, COUNTRY, AIRLINE, PNUMBER, FNUMBER, PRICE, NOTE FROM TOURPRODUCT WHERE TOURNO = ?";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
				pstmt = con.prepareStatement(sql2);
				...
				if(rs.next()) {	
				if (select == loginRs.getInt("TOURNO")) {				
